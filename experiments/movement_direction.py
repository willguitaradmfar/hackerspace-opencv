...
while True:
    ...
    multi = 0.4
    poligono1 = (int(width * multi), height)
    poligono2 = (int(width * multi), 0)
    ...
    for (x, y, w, h) in faces:
        ...
        if len(pontos) > 2:
            ...
            if distx < ErroPadrao and disty < ErroPadrao:
                ...
                elif pontos[0][0] - pontos[1][0] > 0 and (pontos[0][1] - pontos[1][1] >= -10 and pontos[0][1] - pontos[1][1] <= 10):
                    cv2.putText(frame, "Esquerda", (x+(w/2),y+(h/2)), cv2.FONT_HERSHEY_SIMPLEX, 1, 255)
                elif pontos[0][1] - pontos[1][1] > 0 and (pontos[0][0] - pontos[1][0] >= -10 and pontos[0][0] - pontos[1][0] <= 10):
                    cv2.putText(frame, "Baixo", (x+(w/2),y+(h/2)), cv2.FONT_HERSHEY_SIMPLEX, 1, 255)
                elif pontos[0][1] - pontos[1][1] < 0 and (pontos[0][0] - pontos[1][0] >= -10 and pontos[0][0] - pontos[1][0] <= 10):
                    cv2.putText(frame, "Cima", (x+(w/2),y+(h/2)), cv2.FONT_HERSHEY_SIMPLEX, 1, 255)
